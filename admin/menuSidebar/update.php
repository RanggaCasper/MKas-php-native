...
if ($_POST) {
	...
	$menu = $_POST['menu'];
	$access = $_POST['access'];
	$dataUpdate = mysqli_query($conn, "UPDATE user_menu SET menu='$menu' WHERE id_menu='$id'");

	if ($dataUpdate) {
		mysqli_query($conn, "UPDATE user_access_menu SET level='$access' WHERE menu_id = '$id'");
		session_start();
